...
FriendlyChat.prototype.loadMessages = function( patient) {
  ...
  this.messagesRef = this.database.ref('messages/'+currentUser+"/"+ chatuser);
  ...
  var setMessage = function(data) {
    var val = data.val();
   this.displayMessage(data.key, val.from, val.message, val.image, val.time);
  }.bind(this);
...
FriendlyChat.prototype.saveMessage = function(e) {
 ...
 var push_id = firebase.database().ref().child('messages').child(currentUser).child(chatuser).push().key;
    ...
    firebase.database().ref().child('messages').child(chatuser).child(currentUser).child(push_id).set({
       message:this.messageInput.value,
       from:currentUser,
       seen:false,
       image:"https://firebasestorage.googleapis.com/v0/b/patient-management-syste-9758b.appspot.com/o/WOOcyRo2f4PAu7TzKMltwS3fvHa2%2Femployee01%2Fh.png?alt=media&token=ae2d626c-d3cf-47d2-9805-27fa2a42f453",
